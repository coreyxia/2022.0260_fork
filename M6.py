...
def larger_model():
    # create model
    model = Sequential()
    model.add(Conv2D(20, (5, 5), input_shape=(256,256,3), activation='relu'))
    ...
    return model
model=larger_model()
checkpointer = ModelCheckpoint('model-cardboard_M6.h5', verbose=1, save_best_only=True)
results = model.fit(X_train, y_train, validation_data=(X_v, y_v), batch_size=200, epochs=10, 
                    callbacks=[checkpointer])

# Load the previously saved weights
model.load_weights('model-cardboard_M6.h5')
import sklearn.metrics as metrics
y_pred = model.predict(X_test)  
dataset = pd.DataFrame({'M6': y_pred.reshape(-1)})
