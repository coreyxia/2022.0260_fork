...
def larger_model():
    # create model
    cnn4 = Sequential()
    cnn4.add(Conv2D(20, kernel_size=(5, 5), activation='relu', input_shape=(256,256,1)))
...
model=larger_model()
checkpointer = ModelCheckpoint('model-cardboard_M14.h5', verbose=1, save_best_only=True)
results = model.fit(X_train, y_train, validation_data=(X_v, y_v), batch_size=200, epochs=10, 
                    callbacks=[checkpointer])

# Load the previously saved weights
model=larger_model()
model.load_weights('model-cardboard_M14.h5')
import sklearn.metrics as metrics
y_pred = model.predict(X_test)  
dataset = pd.DataFrame({'M14': y_pred.reshape(-1)})
