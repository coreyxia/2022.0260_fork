...
#loading images for DNN
df_train = pd.read_csv(path)
train_image = []
for i in df_train['FileName'] :
    img = image.load_img(path1 + str(i),target_size=(256, 256))
...
#loading image for DNN
df_test = pd.read_csv(path3)
test_image = []
for i in df_test['FileName'] :
    img = image.load_img(path2 + str(i),target_size=(256, 256))
    img = image.img_to_array(img)
    img = img/255
...
def larger_model():
    # create model
    cnn4 = Sequential()
    cnn4.add(Conv2D(20, kernel_size=(5, 5), activation='relu', input_shape=(256,256,3)))
    ...
    # Compile model
    cnn4.compile(loss="mean_squared_error" , optimizer="adam", metrics=["mean_squared_error"])
    return cnn4

model=larger_model()
checkpointer = ModelCheckpoint('model-cardboard_M7.h5', verbose=1, save_best_only=True)
results = model.fit(X_train, y_train, validation_data=(X_v, y_v), batch_size=200, epochs=10, 
                    callbacks=[checkpointer])

# Load the previously saved weights
model=larger_model()
model.load_weights('model-cardboard_M7.h5')
import sklearn.metrics as metrics
y_pred = model.predict(X_test)  
dataset = pd.DataFrame({'M7': y_pred.reshape(-1)})
